...
router bgp {self.onprem_asn}
 bgp router-id {self.onprem_peer_ip}
...
"""
        for i in range(1, self.tunnel_count + 1):
            tunnel_ip = f"169.254.{10+i}.2"
            config += f""" neighbor {tunnel_ip} remote-as {self.aws_asn}
 neighbor {tunnel_ip} timers 10 30 30
 neighbor {tunnel_ip} timers connect 10
 neighbor {tunnel_ip} description AWS-Tunnel{i}
...
"""
        for i in range(1, self.tunnel_count + 1):
            tunnel_ip = f"169.254.{10+i}.2"
            config += f"""  neighbor {tunnel_ip} activate
  neighbor {tunnel_ip} soft-reconfiguration inbound
  neighbor {tunnel_ip} disable-connected-check
"""
...
class VPNManager:
    ...
    def setup(self, interactive: bool = True):
        ...
        print("✅ Configuration files generated in /etc/vpn/")
    ...
    def _gather_config_interactive(self):
        ...
        aws_ips = input("AWS peer IPs (comma-separated): ").strip()
        self.config.aws_peer_ips = [ip.strip() for ip in aws_ips.split(',')]
        
        self.config.onprem_peer_ip = input("On-premises peer IP: ").strip()
    ...
    def monitor(self, auto_fix: bool = True, notify: bool = True):
        ...
        if connectivity['summary']['failed'] > 0:
            ...
            for test in connectivity['tests']:
                if not test['success']:
                    print(f"    ✗ {test['network']}: {test.get('error', 'Unknown')}")
        ...
        if auto_fix and health['issues']:
            ...
            if remediation['fixed']:
                print("\n🔍 Re-checking health after remediation...")
        ...
        try:
            ...
            print(f"\n⚠️  Could not save report to {report_file}")
    ...
    def test_connectivity(self):
        ...
        print("\n🔗 Testing connectivity to remote networks...")
    ...
    def status(self):
        ...
        try:
            ...
            print("  ⚠️  Could not retrieve IPsec status")
        ...
        try:
            ...
            print("  ⚠️  Could not retrieve BGP status")
        ...
        try:
            ...
            print("  ⚠️  Could not retrieve VTI status")
...
Examples:
  ...
  sudo vpn_manager.py --daemon --interval 300
    ...
    if args.setup:
        ...
        print(f"🔄 Starting VPN Manager daemon (interval: {args.interval}s)")
